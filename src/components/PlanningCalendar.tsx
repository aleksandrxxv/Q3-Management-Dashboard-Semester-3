        ...
        fetchAllMaintenance(currentDate, addDays(currentDate, 7), props.mechanic).then((fetchedPlans) => {
            ...
            for (let i = 0; i < 7; i++) {
                weekDaysTemp.push({
                    ...
                    maintenancePlans: fetchedMaintenancePlans.filter(m =>
                        sameDay(m.planned_date, addDays(currentDate, i))
                    ),
                });
            }
    ...
    return (
        <div className={"w-full h-full"}>
            {props.mechanic !== null && mechanic !== null && (
                <div className="flex w-full p-3 bg-orange-100">
                    <span className="block mr-auto">
                        You are now viewing the schedule for {mechanic.name}
                    </span>
                    <a href="/dashboard/maintenance/mechanics" className="">
                        Back
                    </a>
                </div>
            )}
            ...
            <div className="flex gap-2 w-full bg-white px-6 py-4 rounded text-md font-medium border-b items-center">
                <span className="hidden lg:block mr-3">Maintenance schedule</span>
                ...
                <div className="flex gap-2 text-sm items-center mr-auto">
                    ...
                    <span className="block w-24 text-center">
                        {new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(currentDate)}
                    </span>
                ...
                {weekDays.map((weekDay) => (
                    <div key={weekDay.dayDate.getTime()} className="flex w-full">
                        <div className="aspect-square w-40 min-w-40 bg-white p-5 flex flex-col border-b border-r">
                            <span
                                className={
                                    "flex items-center justify-center text-neutral-800 w-9 mb-auto bg-neutral-200 rounded-full aspect-square " +
                                    (new Date().getDate() === weekDay.dayDate.getDate() &&
                                    new Date().getMonth() === weekDay.dayDate.getMonth()
                                        ? "!bg-blue-200"
                                        : "")
                                }
                            >
                                {weekDay.dayDate.getDate()}
                            </span>

                            <div className="block">
                                <span className="block">
                                    {getDayName(weekDay.dayDate)}
                                </span>
                                <span className="block">
                                    {new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(weekDay.dayDate)}
                                </span>
                            </div>
                        </div>
